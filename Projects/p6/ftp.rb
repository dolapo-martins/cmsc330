...
def get(path,socket)
  ...
  if !@logged_in[@s2a[socket].user]
    my_send(socket, "ERRACC")
    return
  end
  my_send(socket, "GOGOGO")
  if path =~ /accounts/ || path=~ /\.\.\// then
...
def put(path,socket)
  ...
  data = my_recv socket
  if data != "IGNORE"
    if path =~ /accounts/ then
      my_send(socket, "ERRACC")
      return
    end
    if path =~ /^public/ || path =~ /\.\.\// then
      full_path = File.join(Dir.pwd,"users",path)
...
def user(arg,socket)
  ...
  if @logged_in.key?(arg)
    my_send(socket, "User attempt in another socket")
    return
  end
  @accounts.each{|a|
    if(a.user==arg)
...
def exitt(socket)
  if @s2a[socket]
    ...
    @logged_in[@s2a[socket].user] = false
    @logged_in.delete(@s2a[socket].user)
    @s2a[socket] = nil
  else
