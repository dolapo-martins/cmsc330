...
*)
let count_x x y =
	fold (fun a h -> (if h = x then a + 1 else a)) 0 y
;;
...
let div_by_x x y =
	(*x or y is zero *)
  map (fun h -> if (x = 0) then false else (h mod x = 0)) y
;;
...
let div_by_first y =
	match y with 
	|[] -> []
	| h::_ -> map (fun z -> if (h = 0) then false else (z mod h = 0)) y
;;
...
*)
let pair_up x y =
 (* map (fun a b -> if (a != [] && b != []) then [a;b] else []) [x;y] *)
  rev (fold (fun acx h -> append (fold (fun acy m -> [h;m]::acy) [] y) acx) [] x)
;;
...
let rec int_to_list t =
	match t with 
	| IntLeaf -> []
	| IntNode (y,l,r) -> append (append (int_to_list l) [y]) (int_to_list r)
;;  
...
let rec int_insert_all xs t =
  fold (fun acc ele -> int_insert ele acc) t xs
;;
...
let rec max_elem t =
  match t with
  	...
  	| IntNode (y,l,r) ->
	  	if r = IntLeaf then y else max_elem r
;;
...
let rec common t x y = 
	...
	| IntNode (h,l,r) -> 
		if 
		((int_mem x l && int_mem y r) || (int_mem x r && int_mem y l) || h = x || h = y) then h
		else if 
			((int_mem x l) && (int_mem y l)) then (common l x y)
		else if
			((int_mem x r) && (int_mem y r)) then (common r x y)
		else
			failwith "common"
;;
(* ===== *)
...
let pinsert x ((f,t):'a ptree) =
	let rec p_insert x t =
		match t with
			| Leaf -> Node(x, Leaf, Leaf)
			| Node (y,l,r) when (f x y) > 0 -> Node (y, l, p_insert x r)
			| Node (y,l,r) when (f x y) < 0 -> Node (y, p_insert x l, r)
			| Node (y,l,r) -> t
	in (f, p_insert x t)
;;
(*| empty_ptree -> ((f, Node(x, empty_ptree, empty_ptree)): 'a ptree)
		| Node (y,((f,l):'a ptree),((f,l):'a ptree)) when (f x y) > 0 -> ((f, Node (y, l, pinsert x ((f,r):'a ptree))): 'a ptree)
		| Node (y,((f,l):'a ptree),((f,l):'a ptree)) when (f x y) < 0 -> ((f, Node (y, pinsert x ((f,l):'a ptree), r)): 'a ptree)
		| Node (y,((f,l):'a ptree),((f,l):'a ptree)) when (f x y) = 0 -> ((f,t): 'a ptree)*)
...
*)   
let pmem x ((f,t):'a ptree) =
   let rec p_mem x t = 
   		match t with
   		| Leaf -> false
   		| Node (y,l,r) when f x y > 0 -> p_mem x r
   		| Node (y,l,r) when f x y < 0 -> p_mem x l
   		| Node (y,l,r) -> true
   	in p_mem x t
;;
...
*)    
let src_edges x g =
  if g = empty_graph then [] else
  	fold (fun acc h -> if h.src = x then h::acc else acc) [] g.edges
;;
...
*)    
let reachable n g =
	if (g = empty_graph) || (src_edges n g = []) then empty_int_tree else (
			let rec visited n g tree =
				let tree = int_insert n tree in
		  			fold (fun acc elem -> if (int_mem elem.dst acc) = false then (int_insert_all (int_to_list(visited elem.dst g acc)) acc) else acc) tree (src_edges n g)
		  	in visited n g empty_int_tree
	  	)
;;