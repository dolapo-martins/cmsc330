class WordNet
	...
	def cost(v, w)
		...
		if (@vertices[v] == @vertices[w]) then 
			...
			lengths = common_lengths.values.sort.first
			#all keys that have same length
			common_lengths.keys.each { |x|
				if (@common_lengths[x] == lengths) && !@LCAs.include?(x) then
					@LCAs << x
				end
			}
	...
	def root(v, w)
		...
		ancestors = ancestor(search_v, search_w)
		#should root only return parents of the minimum length away?
		if ancestors  == -1 then -1 
		elsif ancestors == 0 then
			...
			@vertices.each { |k,v|
				if ancestors.include?(k) == true then
					roots.concat(v)
				end
