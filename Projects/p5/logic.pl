...
prod([], 1). %told to do this -> my preference is commented Out
%prod([], 0).
%prod([X], X).
prod([H|T],R) :-
	prod(T, S),
...
% genN - R is value between 0 and N-1, in order
	%I don't even know how I got this to work
genN(N,R) :-
	N >= 0,
	genN(0, N, R).
genN(X, N, X). %returns next num, then next rule evaluated -> always true, always evaluated
genN(X, N, R) :- %moves to the next
	X < N - 1,
	X1 is X + 1,
	genN(X1, N, R).
...
is_prime(P) :-
	...
	D is floor(sqrt(P)),
	is_prime(P, D).
is_prime(N, N). %if comparing a number to itself, it is prime.
is_prime(N, X) :-
	N > X,
	0 =\= N mod X,
	X1 is X + 1,
