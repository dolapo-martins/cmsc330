...
class TestCase
    ...
    def run(deploy_mode = false)
        if (deploy_mode)
            @@LOGGER.level = Logger::FATAL
        end

        # Dirty refactoring, forgive me senpai
        test_dir = @test_dir
        ...
        # Ensure that every module has its output file unless this is deploy mode
        ...
        @local_resources.each{|resource|
            ...
            FileUtils.cp(@filename_map[resource], get_test_path) unless File.exists?(File.join(get_test_path, File.basename(@filename_map[resource])))
